...
public class Dateien {
    ...
    public static void main(String[] args) {
        ...
        //Hier war ein Fehler in der Pfadangabe
        File bild=new File("..\\..\\..\\KW50_Dateien\\Bilder");
        //Oder auch plattformunabhängig
        File bild2=new File(".."+File.separatorChar+".."+File.separatorChar+
                ".."+ File.separatorChar+"KW50_Dateien");
        ...
        for(File f:dateien){
            System.out.println("Im Verzeichnis Bilder: "+f.getName());
        }
        
        File[] dateien2=bild2.listFiles();
       
        for(File f:dateien2){
            System.out.println("Im Verzeichnis KW50_Dateien: "+f.getName());
        }
        File loeschbar=new File("test.txt");
        if(!loeschbar.exists()){
            System.out.println("Gibt es noch nicht");
            try {
                loeschbar.createNewFile();
            } catch (IOException ex) {
                System.out.println("Datei konnte nicht angelegt werden");
            }
        }
        loeschbar.delete();
        File verz=new File("ordner");
        if(!verz.exists()){
            verz.mkdir();
        }
        System.out.println("Îstz es eines? "+verz.isDirectory());
